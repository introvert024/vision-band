...
def receive_video():
    ...
    try:
        while True:
            ...
            object_detected = process_frame(frame, args, net, COLORS, CLASSES)

            # Send object detection result back to Raspberry Pi
            connection.send(object_detected.encode())

    finally:
        connection.close()
        server_socket.close()

def process_frame(frame, args, net, COLORS, CLASSES):
